...
function MainApp() {
  ...
  useEffect(() => {
    const loadUserData = () => {
      try {
        const storedData = localStorage.getItem('cosmetics_backup');
        if (storedData) {
  ...
  useEffect(() => {
    ...
    const saveUserData = () => {
      try {
        ...
        localStorage.setItem('cosmetics_backup', JSON.stringify(dataToSave));
      } catch (err: any) {
    ...
    saveUserData();
  }, [categories, products, capacityUnits, isDataLoaded]);

  const [apiKeys, setApiKeys] = useState<string[]>(() => {
    let keys = ['', '', ''];
    try {
      const stored = localStorage.getItem(`cosmetics_gemini_api_keys`);
      if (stored) {
  ...
  const handleSaveApiKey = () => {
    localStorage.setItem(`cosmetics_gemini_api_keys`, JSON.stringify(apiKeyInputs));
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (json.categories && json.products) {
          ...
          localStorage.setItem('cosmetics_backup', JSON.stringify({
            categories: json.categories,
            capacityUnits: json.capacityUnits || capacityUnits,
